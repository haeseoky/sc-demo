...
public class LocalCircuitBreaker {
    ...
    private CircuitBreaker setCircuitBreaker() {
        ...
        circuitBreaker1
            ...
            .onStateTransition(event -> {
                log.info("{}: [CircuitBreaker] onStateTransition: {}", CIRCUIT_BREAKER_NAME, event);
            })
    ...
    public void logPrintCount() {
        log.info("[CircuitBreaker] NumberOfBufferedCalls: {}, NumberOfSuccessfulCalls: {}, NumberOfFailedCalls: {}, NumberOfNotPermittedCalls: {}, FailureRate: {}", circuitBreaker.getMetrics().getNumberOfBufferedCalls(), circuitBreaker.getMetrics().getNumberOfSuccessfulCalls(), circuitBreaker.getMetrics().getNumberOfFailedCalls(), circuitBreaker.getMetrics().getNumberOfNotPermittedCalls(), circuitBreaker.getMetrics().getFailureRate());
